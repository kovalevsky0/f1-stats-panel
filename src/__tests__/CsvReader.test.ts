...
describe("CsvReader", () => {
  ...
  it("method `read` reads data from file correctly", () => {
    ...
    // date
    expect(csvReader.data[0][0].toDateString()).toBe("Mon Jun 01 2020");
    // racetrack
    ...
    // points
    expect(csvReader.data[0][4]).toEqual(expect.any(Number));
    expect(csvReader.data[0][4]).toBe(72);

    //console.log(csvReader.data);
  });
  it("method `parseDate` returns parsed date", () => {
    expect(csvReader.parseDate("1/6/2020").toDateString()).toBe(
      "Mon Jun 01 2020"
    );
  });
});
